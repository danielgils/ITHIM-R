---
title: "Trip Summary ITHIM-R Latinamerican Cities"
author: ""
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: html_document
---


```{r, include = FALSE}
source("used_functions.R")
knitr::opts_chunk$set(comment = NA, prompt = FALSE, cache = FALSE,echo = F,
                      results = 'asis')
```

```{r loadLibraries, echo = F, message = F}

st_options(bootstrap.css     = FALSE,       # Already part of the theme so no need for it
           plain.ascii       = FALSE,       # One of the essential settings
           style             = "rmarkdown", # Idem.
           dfSummary.silent  = TRUE,        # Suppresses messages about temporary files
           footnote          = NA,          # Keeping the results minimalistic
           subtitle.emphasis = FALSE)       # For the vignette theme, this gives
                                            # much better results. Your mileage may vary

```

```{r,  comment=NA, message=FALSE, warning=FALSE, prompt=FALSE, cache=FALSE, echo=F}
# trip <- read_csv("../../data/local/buenos_aires/buenos_aires_trip.csv")
# quality_check(trip)
# trip_summary$Buenos_Aires <- value
# 
# trip <- read_csv("../../data/local/sao_paulo/sao_paulo_trip.csv")
# quality_check(trip)
# trip_summary$Sao_Paulo <- value
# 
# trip <- read_csv("../../data/local/belo_horizonte/belo_horizonte_trip.csv")
# quality_check(trip)
# trip_summary$Belo_Horizonte <- value
# 

# 
trip <- read_csv("../../data/local/bogota/bogota_trip.csv")
quality_check(trip)
trip_summary$Bogota <- value

trip <- read_csv("../../data/local/bogota_wb2/bogota_trips_wb2.csv")
quality_check(trip)
trip_summary$Bogota_2019 <- value

trip <- read_csv("../../data/local/bogota_wb2/bogota_trips_longer15_wb2.csv")
quality_check(trip)
trip_summary$Bogota_2019_longer15 <- value
# 
# trip <- read_csv("../../data/local/cali_wb/trips_cali_wb.csv")
# quality_check(trip)
# trip_summary$Cali_wb <- value
# 
# trip <- read_csv("../../data/local/cali_wb/trips_cali_wb.csv")
# quality_check(trip)
# trip_summary$Cali_wb <- value
# 
# trip <- read_csv("../../data/local/medellin_wb/trips_medellin_wb.csv")
# quality_check(trip)
# trip_summary$Medellin_wb <- value
# 
# trip <- read_csv("../../data/local/medellin_wb/trips_medellin_wb.csv")
# quality_check(trip)
# trip_summary$Medellin_wb <- value
# 
trip <- read_csv("../../data/local/mexico_city/mexico_city_trip.csv")
quality_check(trip)
trip_summary$Mexico <- value
 
trip <- read_csv("../../data/local/mexico_city_wb/mexico_city_trips_weekday_wb.csv")
quality_check(trip)
trip_summary$Mexico_city_weekdays <- value

trip <- read_csv("../../data/local/mexico_city_wb/mexico_city_trips_weekend_wb.csv")
quality_check(trip)
trip_summary$Mexico_city_weekend <- value
# 
# trip <- read_csv("../../data/local/santiago/santiago_trip.csv")
# quality_check(trip)
# trip_summary$Santiago <- value
# 
# trip <- read_csv("../../data/local/vizag/vizag_trip.csv")
# quality_check(trip)
# trip_summary$Visakhapatnam <- value
# 
# trip <- read_csv("../../data/local/delhi/delhi_trip.csv")
# quality_check(trip)
# trip_summary$Delhi <- value
# 
# trip <- read_csv("../../data/local/bangalore/bangalore_trip.csv")
# quality_check(trip)
# trip_summary$Bangalore <- value
# 
# trip <- read_csv("../../data/local/accra/accra_trip.csv")
# quality_check(trip)
# trip_summary$Accra <- value
# 
# trip <- read_csv("../../data/local/cape_town/cape_town_trip.csv")
# quality_check(trip)
# trip_summary$Cape_Town <- value

```


```{r, include = TRUE,  echo= FALSE, results='asis'}
kable(trip_summary) %>%
kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), font_size = 14, fixed_thead = T)
    
```
